...
Lua脚本的编译执行是相互独立的，在不同的线程上执行。通过luaL_newstate()函数可以申请一个虚拟机，返回指针类型 lua_State。
...
另一方面，长期运行的程序，比如一个后台程序或是一个 web 服务器，当不再需要它们的时候就应该释放掉相关状态机。这样可以避免状态机扩张的过大。
lua_close(L);
-------------------------------------------------------------------------------------- -
...
扩大栈的可用尺寸，栈的默认尺寸是20，此函数会确保堆栈上至少有 sz 个空位。如果不能把堆栈扩展到相应的尺寸，函数返回 false 。
...
lua_tointeger(L, -1)		-->200		// 注：idx为正表示相对于栈底的偏移量，为负表示相对于栈顶的偏移量（即-1表示栈顶元素）
lua_pushboolean(L, 0)
...
int   lua_pcall(lua_State *L, int nargs, int nresults, int errfunc)	//nargs参数个数， nresults返回值个数， errfunc错误处理时的代码
-------------------------------------------------------------------------------------- -
...
lua_pushstring(L, "onebyone");		-->'this' 'true' '9989' '1111' 'false' '满天都是小星星' '1986' 'onebyone'
lua_concat(L, 3);					-->'this' 'true' '9989' '1111' 'false' '满天都是小星星1986onebyone'
-------------------------------------------------------------------------------------- -
